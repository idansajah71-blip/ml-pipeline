  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Ensemble Models</h1>
          <p className="text-gray-500 dark:text-gray-400">Combine multiple models for better predictions</p>
        </div>
        <button onClick={() => setShowCreate(true)} className="flex items-center gap-2 rounded-lg bg-primary-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-primary-700">
          <Plus className="h-4 w-4" /> New Ensemble
        </button>
      </div>

      {showCreate && (
        <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6">
          <h2 className="mb-4 text-lg font-semibold text-gray-900 dark:text-white">Create Ensemble</h2>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <input placeholder="Ensemble Name" value={newEnsemble.name} onChange={e => setNewEnsemble({...newEnsemble, name: e.target.value})}
          ...
          <div className="mt-4">
            <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Select Models (min 2):</p>
          ...
          <div className="mt-4 flex gap-2">
            <button onClick={createEnsemble} className="rounded-lg bg-primary-600 px-4 py-2 text-sm font-medium text-white hover:bg-primary-700">Create</button>
            <button onClick={() => setShowCreate(false)} className="rounded-lg border border-gray-300 px-4 py-2 text-sm text-gray-700">Cancel</button>
          </div>
        </div>
      )}
      ...
      {loading ? <LoadingSpinner size="lg" className="mx-auto" /> : (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6">
            <h2 className="mb-4 text-lg font-semibold text-gray-900 dark:text-white">Ensembles</h2>
            <div className="space-y-2">
              {ensembles.map(ens => (
                <button key={ens.id} onClick={() => setPredictEnsemble(ens.id)}
                  ...
                  <p className="text-xs text-gray-500">{ens.strategy} | {ens.model_ids.length} models</p>
                </button>
              ))}
              {ensembles.length === 0 && <p className="text-sm text-gray-500">No ensembles yet</p>}
            </div>
          </div>

          {predictEnsemble && (
            <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-6">
              <h2 className="mb-4 text-lg font-semibold text-gray-900 dark:text-white">Test Ensemble</h2>
